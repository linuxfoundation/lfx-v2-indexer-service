...
// GroupsIOMailingListSettingsEnricher handles groupsio-mailing-list-settings-specific enrichment logic
type GroupsIOMailingListSettingsEnricher struct {
	defaultEnricher Enricher
}

// ObjectType returns the object type this enricher handles.
func (e *GroupsIOMailingListSettingsEnricher) ObjectType() string {
	return e.defaultEnricher.ObjectType()
}

// EnrichData enriches groupsio-mailing-list-settings-specific data
func (e *GroupsIOMailingListSettingsEnricher) EnrichData(body *contracts.TransactionBody, transaction *contracts.LFXTransaction) error {
	return e.defaultEnricher.EnrichData(body, transaction)
}

// setAccessControl provides groupsio-mailing-list-settings-specific access control logic
// Permissions are based on the parent groupsio_mailing_list object
func (e *GroupsIOMailingListSettingsEnricher) setAccessControl(body *contracts.TransactionBody, data map[string]any, objectType, objectID string) {
	// Build access control values
	var accessObject, accessRelation string
	var historyObject, historyRelation string

	// Set access control with groupsio-mailing-list-settings-specific logic
	...
	if accessCheckObject, ok := data["accessCheckObject"].(string); ok {
		...
		// Field doesn't exist in data - use computed default with parent object type
		accessObject = fmt.Sprintf("groupsio_mailing_list:%s", objectID)
	}
	...
	if accessCheckRelation, ok := data["accessCheckRelation"].(string); ok {
		...
		accessRelation = "auditor" // Settings default
	}
	...
	if historyCheckObject, ok := data["historyCheckObject"].(string); ok {
		...
		historyObject = fmt.Sprintf("groupsio_mailing_list:%s", objectID)
	}
...
// NewGroupsIOMailingListSettingsEnricher creates a new groupsio mailing list settings enricher
func NewGroupsIOMailingListSettingsEnricher() Enricher {
	enricher := &GroupsIOMailingListSettingsEnricher{}
	enricher.defaultEnricher = newDefaultEnricher(
		constants.ObjectTypeGroupsIOMailingListSettings,
