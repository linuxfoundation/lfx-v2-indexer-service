...
// GroupsIOServiceSettingsEnricher handles groupsio-service-settings-specific enrichment logic
type GroupsIOServiceSettingsEnricher struct {
	defaultEnricher Enricher
}

// ObjectType returns the object type this enricher handles.
func (e *GroupsIOServiceSettingsEnricher) ObjectType() string {
	return e.defaultEnricher.ObjectType()
}

// EnrichData enriches groupsio-service-settings-specific data
func (e *GroupsIOServiceSettingsEnricher) EnrichData(body *contracts.TransactionBody, transaction *contracts.LFXTransaction) error {
	return e.defaultEnricher.EnrichData(body, transaction)
}

// setAccessControl provides groupsio-service-settings-specific access control logic
// Permissions are based on the parent groupsio_service object
func (e *GroupsIOServiceSettingsEnricher) setAccessControl(body *contracts.TransactionBody, data map[string]any, objectType, objectID string) {
	// Build access control values
	var accessObject, accessRelation string
	var historyObject, historyRelation string

	// Set access control with groupsio-service-settings-specific logic
	...
	if accessCheckObject, ok := data["accessCheckObject"].(string); ok {
		...
		accessObject = fmt.Sprintf("groupsio_service:%s", objectID)
	}
	...
	if historyCheckObject, ok := data["historyCheckObject"].(string); ok {
		...
		historyObject = fmt.Sprintf("groupsio_service:%s", objectID)
	}
...
// NewGroupsIOServiceSettingsEnricher creates a new groupsio service settings enricher
func NewGroupsIOServiceSettingsEnricher() Enricher {
	enricher := &GroupsIOServiceSettingsEnricher{}
	enricher.defaultEnricher = newDefaultEnricher(
		constants.ObjectTypeGroupsIOServiceSettings,
